...
class AuthController {
  ...
  async verifyResetCode(req, res) {
    ...
    if (!email || !code) {
      return res
        .status(400)
        .json({ success: false, error: "Thiếu email hoặc mã xác nhận." });
    }
    ...
    try {
      ...
      if (
        ...
        return res
          .status(400)
          .json({ success: false, error: "Mã không hợp lệ hoặc đã hết hạn." });
      }
      ...
      res
        .status(500)
        .json({ success: false, error: "Lỗi máy chủ khi kiểm tra mã." });
    }
  }
  ...
  async resetPassword(req, res) {
    ...
    try {
      ...
      res
        .status(200)
        .json({ success: true, message: "Mật khẩu đã được đặt lại." });
    } catch (err) {
      res
        .status(500)
        .json({ success: false, error: "Lỗi máy chủ khi đặt lại mật khẩu." });
    }
  }
}
